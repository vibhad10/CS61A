...
def choose(paragraphs, select, k):
    ...
    """
    # BEGIN PROBLEM 1
    new_lst = [paragraph for paragraph in paragraphs if select(paragraph)]
    if len(new_lst) <= k:
        return ''
    else:
        return new_lst[k]
...
def about(topic):
    ...
    # BEGIN PROBLEM 2
    def word_in(paragraph):
        paragraph = paragraph.lower()
        paragraph = remove_punctuation(paragraph)
        new_lst = paragraph.split()
...
def autocorrect(typed_word, valid_words, diff_function, limit):
    ...
    if typed_word in valid_words:
        ...
        min_value = min(valid_words, key=lambda word: diff_function(typed_word, word, limit))
        if diff_function(typed_word, min_value, limit) <= limit:
            return min_value
        else: 
            return typed_word
...
def minimum_mewtations(start, goal, limit):
    ...
    3
    """
    #print('DEBUG', f'start = {start}, goal = {goal}, limit = {limit}')
    ...
    if start == goal:
        ...
        limit -= 1
        add = minimum_mewtations(goal[0] + start, goal, limit)
        remove = minimum_mewtations(start[1:], goal, limit)
        substitute = minimum_mewtations(goal[0] + start[1:], goal, limit )
...
def time_per_word(words, times_per_player):
    ...
    for timestamps in times_per_player:
        time_entry = []
        for index0 in range(len(words)):
            time_entry.append(timestamps[index0 + 1] - timestamps[index0])
...
def fastest_words(match):
    ...
    # BEGIN PROBLEM 10
    retval = [[] for _ in player_indices]
    for word_index in word_indices:
        ...
        min_index = player_times.index(min_player_time)
        retval[min_index].append(word_at(match, word_index))
    return retval 
    # END PROBLEM 10
