...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        while not place.is_hive:
            if self.min_range <= steps <= self.max_range and place.bees:
...
class TankAnt(ContainerAnt):
    ...
    def action(self, group):
        bee_list = self.place.bees[:]

        BodyguardAnt.action(self, group)
...
# END Problem 12
    ...
    def construct(cls, gamestate):
        ...
        if cls.food_cost > gamestate.food:
            print('Not enough food remains to place ' + cls.__name__)
            return
        if not cls.queen_added:
            #construct(gamestate)
            cls.queen_added = True
